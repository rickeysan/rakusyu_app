...
//     debug('$resultの中身print_r：'.print_r($stmt,true));
//     var_dump($stmt);
//     if($stmt){
//         debug('クエリ成功');
//         debug($sql.'の実行に成功しました');
//         return $stmt;
//     }else{
//         debug('クエリ失敗');
//         debug($sql.'の実行に失敗しました');
//         return 0;
//     }
// }
...
//　ページング用のGETパラメータ付きURL生成関数
function makePagingURL($str){
    if(!empty($_GET['p'])){
        if(appendGetParam()){
...
// 画像用のフォーム入力保持関数
// function getFormDataImg($form_data,$key,$pic_pass){
//     debug('画像用のフォーム入力保持関数');
//     global $err_msg;
